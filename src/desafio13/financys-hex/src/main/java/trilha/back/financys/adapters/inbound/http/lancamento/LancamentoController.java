...
public class LancamentoController {
    ...
    public ResponseEntity<List<LancamentoResponse>> read(){
        ...
        lancamentoResponseLista.sort(Comparator.comparing(LancamentoResponse::getDate));
        ...
        categoriaLista.stream()
                .forEach(
                        categoria -> {
                            ...
                            lancamentoChart.setName(categoria.getName());

                            lancamentoLista
                                    .stream()
                                    .filter(lancamento -> lancamento.getCategoryId().getId() == categoria.getId())
                                    .forEach(
                                            lancamento -> {
                                                Double montante = lancamento.getAmount();

                                                total.updateAndGet(valor -> valor + montante);

                                                lancamentoChart.setType(lancamento.getType());
